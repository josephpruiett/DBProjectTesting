...
iod.DiagnosisCodeId
into #tmp_claim_diag
from [$(DatabaseName)].dbo.Claim ioc
inner join [$(DatabaseName)].dbo.ServiceLine iosl
on (
ioc.FileRequestId = @FileRequestId
and iosl.Claim_id = ioc.ClaimId
)
inner join [$(DatabaseName)].dbo.DiagnosisCode iod
on (
...
from [$(DatabaseName)].dbo.DiagnosisCode iod
