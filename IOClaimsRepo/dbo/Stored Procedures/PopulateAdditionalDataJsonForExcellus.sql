--CREATE TYPE ClaimNumberType AS TABLE (ClaimNumber VARCHAR(50))

CREATE PROCEDURE PopulateAdditionalDataJsonForExcellus
	@ClaimList ClaimNumberType READONLY
AS 

	
BEGIN

	SELECT ClaimNumber from @ClaimList
	
	Print 'Updating Claim Json'
	...
	FROM ClaimAdditionalData ) AS B ON A.ClaimID = B.ClaimID 	
	WHERE 
		A.AdditionalDataJSON is null 
		and A.ClaimNumber in (SELECT ClaimNumber from @ClaimList)
		and A.ClientId = 3 --Excellus


	Print 'Updating ServiceLine Json'

	UPDATE A Set 
	--update mods, while we are at it - this causes nullref on AP
	ModifierCode1 = Coalesce(ModifierCode1,''), 
	ModifierCode2 = Coalesce(ModifierCode2,''),
	ModifierCode3= Coalesce(ModifierCode3,''),
	ModifierCode4= Coalesce(ModifierCode4,''),

	AdditionalDataJSON = B.AdditionalDataJSON 
	FROM [ServiceLine] A INNER JOIN (
	...
	WHERE 
		A.Claim_ID in 
		(
		SELECT ClaimId from Claim A  
		WHERE 
		A.ClaimNumber in (SELECT ClaimNumber from @ClaimList)
		and A.ClientId = 3) --Excellus
		and A.AdditionalDataJson is null



	Print 'Updating Bene Json'



	UPDATE A Set 
	--update address, while we are at it - this causes nullref on AP
	...
	+'}'
	,'"null"','null') as AdditionalDataJSON
	FROM BeneficiaryAdditionalData ) AS B ON A.BeneficiaryID = B.BeneficiaryID 
	WHERE 
		A.AdditionalDataJSON is null 
		and B.BeneficiaryId in (SELECT distinct Beneficiary_Id from Claim where ClaimNumber in (SELECT ClaimNumber from @ClaimList))
		and A.ClientId = 3 --Excellus


	Print 'Updating Provider Json'
	...
	+'}'
	,'"null"','null') as AdditionalDataJSON
	FROM ProviderAdditionalData ) AS B ON A.ProviderID = B.ProviderID 
	WHERE 
	A.ProviderId in (
			SELECT distinct AttendingProvider_id as ProviderId from Claim 
			where AttendingProvider_id is not null and ClaimNumber in (SELECT ClaimNumber from @ClaimList)
		union all
			SELECT distinct BillingProvider_id as ProviderId from Claim 
			where BillingProvider_id is not null and ClaimNumber in (SELECT ClaimNumber from @ClaimList)
		union all	
			SELECT distinct ReferringProviderOrOtherSourceProvider_id as ProviderId from Claim 
			where ReferringProviderOrOtherSourceProvider_id is not null and ClaimNumber in (SELECT ClaimNumber from @ClaimList)
	)
